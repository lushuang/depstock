package cn.com.depstock.controller.fhdb.brdb;

import cn.com.depstock.controller.base.BaseController;
import cn.com.depstock.entity.Page;
import cn.com.depstock.service.fhdb.brdb.BRdbManager;
...
public class BRdbController extends BaseController {
    ...
    public ModelAndView listAllTable() throws Exception {
        ...
        Object[] arrOb = DBDepstock.getTables();
    ...
    public Object backupAll() {
        ...
        String kackupPath;
        try {
            kackupPath = DBDepstock.getDBDepstock().backup("").toString();        //调用数据库备份
    ...
    public Object backupTable() {
        ...
        String kackupPath;
        try {
            kackupPath = DBDepstock.getDBDepstock().backup(TABLENAME).toString();    //调用数据库备份
            if (Tools.notEmpty(kackupPath) && !"errer".equals(kackupPath)) {
                pd.put("FHDB_ID", this.get32UUID());                //主键
                pd.put("USERNAME", username);                        //操作用户
                pd.put("BACKUP_TIME", Tools.date2Str(new Date()));    //备份时间
    ...
    public Object dbRecover() {
        ...
        try {
            String returnStr = DBDepstock.getDBDepstock().recover(TABLENAME, SQLPATH).toString();
    ...
    public ModelAndView list(Page page) throws Exception {
        ...
        Map<String, String> DBmap = DBDepstock.getDBParameter();
