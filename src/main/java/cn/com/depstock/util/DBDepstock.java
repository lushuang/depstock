 ...
 */
public class DBDepstock {
    private static Log logger = LogFactory.getLog(DBDepstock.class);
    ...
    private static DBDepstock DBDepstock = new DBDepstock();

    public static void main(String[] arg) {
        try {
            String str = DBDepstock.getDBDepstock().backup("").toString();//调用数据库备份
    ...
    public static DBDepstock getDBDepstock() {
        return DBDepstock;
    }
    ...
    public static Object[] getTables(PageData pd) throws ClassNotFoundException, SQLException {
        ...
        Connection conn = DBDepstock.getCon(dbtype, username, password, address + ":" + dbport, databaseName);
        if ("oracle".equals(dbtype)) {
            databaseName = username;
        }
        Object[] arrOb = {databaseName, DBDepstock.getTablesByCon(conn, "sqlserver".equals(dbtype) ? null : databaseName), dbtype};
        return arrOb;
    }
    ...
    public static Connection getFHCon() throws ClassNotFoundException, SQLException {
        ...
        String dbport = pros.getProperty("dbport");                //端口
        String databaseName = pros.getProperty("databaseName");    //数据库名
        return DBDepstock.getCon(dbtype, username, password, address + ":" + dbport, databaseName);
    }
            ...
            try {
                ...
                if (!"sqlserver".equals(dbtype)) {
                    ...
                    if ("yes".equals(remoteDB)) {//数据库另外一台服务器上(和tomcat不在同一台服务器上)
                        commandStr = DBDepstock.getExecStr(dbtype, dbpath, "localhost", username, password, sqlpath, tableName, databaseName, ffilename); //命令语句
                        Socket ss = null;
                        ...
                        commandStr = DBDepstock.getExecStr(dbtype, dbpath, address, username, password, sqlpath, tableName, databaseName, ffilename); //命令语句
                    ...
                    PreparedStatement bak = DBDepstock.getFHCon().prepareStatement(bakSQL);
        ...
        public Object call() {
            try {
                ...
                if (!"sqlserver".equals(dbtype)) {
                    ...
                    PreparedStatement ps = DBDepstock.getFHCon().prepareStatement(recoverySql);
			        CallableStatement cs = DBDepstock.getFHCon().prepareCall("{call killrestore(?,?)}");
                    ...
                    PreparedStatement recovery = DBDepstock.getFHCon().prepareStatement(reSQL);
    ...
    public static Object[] executeQueryFH(String sql) throws Exception {
        ...
        conn = DBDepstock.getFHCon();
        stmt = conn.createStatement();
        rs = stmt.executeQuery(sql);
        columnList = DBDepstock.getFieldLsit(conn, sql);
        while (rs.next()) {
    ...
    /**
     * 读取db-depstock.properties配置文件
     *
     * @return
     * @throws IOException
     */
    public static Properties getPprVue() {
        InputStream inputStream = DBDepstock.class.getClassLoader().getResourceAsStream("db-depstock.properties");
